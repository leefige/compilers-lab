...
public class RedunNullCheck implements Flow.Analysis {
    ...
    private static HashMap<String, TreeSet<Integer>> checkMap;
    /**
     ...
     */
    private QuadSet[] in, out;
    private QuadSet entry, exit;
    ...
    public void preprocess(ControlFlowGraph cfg) {
        ...
        checkMap = new HashMap<String, TreeSet<Integer>>();
        ...
        while (qit.hasNext()) {
            ...
            if (oprt instanceof Operator.NullCheck){
                for (RegisterOperand target : q.getUsedRegisters()) {
                    String reg = target.toString();
                    if (!checkMap.containsKey(reg)) {
                        checkMap.put(reg, new TreeSet<Integer>());
                    }
                    checkMap.get(reg).add(id);
                }
            }
        }
        // initialize the entry and exit points.
        transferfn.val = new QuadSet();
        entry = new QuadSet();
        exit = new QuadSet();
        System.out.println("Init finished. checkMap: ");
    }
    ...
    public static class TransferFunction extends QuadVisitor.EmptyVisitor {
        ...
        public void visitQuad(Quad q) {
            for (RegisterOperand def : q.getDefinedRegisters()) {
                ...
                Iterator<Integer> iter = checkMap.get(key).iterator();
                while (iter.hasNext())
